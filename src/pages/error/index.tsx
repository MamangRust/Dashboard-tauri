...
export default function ErrorPage() {
  ...
  const handleRefresh = () => {
    window.location.reload() // Use window.location to refresh the page
  }

  const handleBack = () => {
    navigate(-1) // Go back to the previous page using React Router
  }
    ...
    <div className="flex items-center justify-center min-h-screen bg-background p-4">
      <Card className="w-full max-w-md">
        ...
        <CardFooter className="flex flex-col space-y-2 sm:flex-row sm:space-x-2 sm:space-y-0">
          ...
          <Button onClick={copyErrorMessage} variant="secondary" className="w-full sm:w-auto">
            Copy Error
          </Button>
        </CardFooter>
      </Card>
